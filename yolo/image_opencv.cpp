...
static int user_flag = 0; //내가 만든 flag
...
image get_image_from_stream_cpp(cap_cv *cap)  // 이걸 사용해야할 거 같음.
{
...
void save_mat_png(cv::Mat img_src, const char *name)  //이것도 있고
{
    cv::Mat img_rgb;
...
void save_mat_jpg(cv::Mat img_src, const char *name)
{
    cv::Mat img_rgb;
    if (img_src.channels() >= 3) cv::cvtColor(img_src, img_rgb, cv::COLOR_RGB2BGR);
    stbi_write_jpg(name, img_rgb.cols, img_rgb.rows, 3, (char *)img_rgb.data, 100);
}
// ----------------------------------------
...
// 추가 코드
                    /*printf("%d", names[j] == "person");
                    printf("%s", names[j]);
                    printf("%d", strcmp(names[j], "person"));*/
                    if (strcmp(names[j],"person") == 0 && dets[i].prob[j]*100 > 98 &&user_flag < 100) user_flag++;   //5번 person 인식하면 그림파일 저장
                    printf("user_flag: %d", user_flag);
                    if (user_flag == 100) {
                        user_flag=0;
                        time_t now = time(NULL);
                        char *now_time = ctime(&now);
                        string datapath = "C:\\Bitnami\\wampstack-7.1.19-0\\apache2\\htdocs\\src\\";

                        string str(now_time);
                        str.replace(24, 1, ".jpg");
                        // ':' 를 ' '으로 바꾼다.
                        str.replace(13, 1, " ");
                        str.replace(16, 1, " ");
                        datapath.append(str); // 경로\time.jpg
                        strcpy(now_time, datapath.c_str());
                        //printf("%s\n", now_time);
                        //printf("%s\n", ctime(&now));
                        //str.append(".jpg");
                        //std::cout << str << "\n";
                        save_mat_jpg(*show_img, now_time);
                        printf("success to save jpg");
//추가코드
                    }
                }
            }
